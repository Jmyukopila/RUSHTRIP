...
- `services/*.py` — toda la lógica y las llamadas a APIs externas. `services/plan.py::generar_plan()` es el orquestador: resuelve ciudad→IATA (`resolver_iata`), combina vuelos+hoteles+coches, filtra por tier y elige el plan más caro que quepa en el presupuesto. Soporta `medio_transporte` (`avion|bus|tren`): con bus/tren usa `services/transport.py` (precios estimados por distancia, sin API real; mismo contrato que un vuelo + campos `medio` y `distancia_km`, va en la key `vuelo` del plan) y si la ruta excede el límite del medio (bus 1.500 km, tren 2.500 km) o no hay coords, **degrada a avión con `aviso`**, nunca falla.
- `core/` — infraestructura: `http.py` (cliente httpx compartido + `request_with_retry` con backoff y throttle de salida por proveedor — `_THROTTLE_CONFIG` — que lanza `ExternalAPIError`; toda llamada externa nueva debe pasar por `request_with_retry` con su `provider`), `database_cache.py` (caché SQLite WAL en `cache/`: `cache_get` / `cache_get_stale` / `cache_set(key, value, provider, ttl_seconds)`; lo vencido se retiene 7 días para el fallback stale), `cache.py` (TTLCache en memoria), `rate_limiter.py` (límites diarios por IP real — `X-Forwarded-For` tras proxy, localhost exento — solo sobre rutas de la API, persistentes), `errors.py` (jerarquía `AppError`), `config.py` (Pydantic Settings desde `.env`, soporta rotación multi-key `*_2`, `*_3`).
...
- **Iconos**: SVG inline pequeños, sin emoji. Las banderas de país en `AirportInput` (`FlagIcon`) se renderizan como imágenes de flagcdn (`https://flagcdn.com/24x18/{iso2}.png`) a partir del `pais_codigo` ISO-2 que devuelve `/airports/`; no usar emoji de bandera porque los navegadores en Windows los muestran como el código ISO en texto. Set compartido en `frontend/src/components/icons.jsx` (`IconWarning`, `IconCheckCircle`, `IconStarRow`, `ACTIVITY_ICON_MAP`, `WEATHER_ICON_MAP`, etc.) — reusar antes de crear uno nuevo. `ACTIVITY_ICON_MAP`/`WEATHER_ICON_MAP` traducen los campos `categoria`/`icono` (emoji) que devuelve el backend a iconos locales; si el backend agrega una categoría o código climático nuevo, sumarlo ahí. No añadir librerías de iconos.
